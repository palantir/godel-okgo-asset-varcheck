...
`
	...
	assetPath, err := products.Bin("varcheck-asset")
...
`,
				...
				WantError:   true,
				WantOutput: `Running varcheck...
foo.go:3:5: fooNum
Finished varcheck
Check(s) produced output: [varcheck]
`,
			},
			{
...
`,
	...
	godel/config/check-plugin.yml: failed to upgrade configuration: failed to upgrade check "varcheck" legacy configuration: failed to upgrade asset configuration: varcheck-asset does not support legacy configuration with a non-empty "args" field
`,
				WantFiles: map[string]string{
					"godel/config/check.yml": `
checks:
...
`,
				...
				WantError: true,
				WantOutput: `Failed to upgrade configuration:
	godel/config/check-plugin.yml: failed to upgrade check "varcheck" configuration: failed to upgrade asset configuration: varcheck-asset does not currently support configuration
`,
				WantFiles: map[string]string{
					"godel/config/check-plugin.yml": `
checks:
